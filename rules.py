...
def total_revenue(data: dict, financial_index):
    ...
    pnl_section = financial_entry["pnl"]["lineItems"]
    return pnl_section.get("netRevenue", 0.0)
...
def total_borrowing(data: dict, financial_index):
    ...
    """
    financial_entry = data["financials"][financial_index]
    bs_section = financial_entry["bs"]
    total_borrowings = bs_section.get("long_term_borrowings", 0.0) + bs_section.get("short_term_borrowings", 0.0)
...
def iscr(data: dict, financial_index):
    ...
    financial_entry = data["financials"][financial_index]
    pnl_section = financial_entry["pnl"]["lineItems"]
    pbit = pnl_section.get("profit_before_interest_and_tax", 0.0)
    depreciation = pnl_section.get("depreciation", 0.0)
    interest_expenses = pnl_section.get("interest", 0.0)
    return (pbit + depreciation + 1) / (interest_expenses + 1)
...
def borrowing_to_revenue_flag(data: dict, financial_index):
    ...
    ratio = total_borrowings / total_rev if total_rev != 0 else 0.0
